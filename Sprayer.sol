 ...
 * _Available since v3.4._
 */
contract Sprayer is EIP712, Ownable {
    ...
    /**
     * @dev Initializes the {EIP712} domain separator using the `name` "Orimatla" parameter, and setting `version` to `"1"`.
     */
    constructor(IERC20 token_) EIP712("Orimatla", "1") {
        _token = token_;
    }
    ...
    /**
     * @dev Transfers `amount` tokens held by the Sprayer contract to the `account`,
    ...
    function withdraw(
        ...
        require(block.timestamp <= deadline, "Sprayer: expired deadline");
        address owner = owner();
        bytes32 structHash = keccak256(abi.encode(_PERMIT_TYPEHASH, account, amount, _useNonce(account), deadline));

        bytes32 hash = _hashTypedDataV4(structHash);

        address signer = ECDSA.recover(ECDSA.toEthSignedMessageHash(hash), v, r, s);
        require(signer == owner, "Sprayer: invalid signature");
    ...
    /**
     * @dev Transfers all tokens held to `owner`
     */
    function withdrawToOwner() public virtual onlyOwner returns (bool) {
